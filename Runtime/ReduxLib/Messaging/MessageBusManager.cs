...
public static class MessageBusManager
{
    ...
    /// <exception cref="Exception">Thrown when the given name is null or empty.</exception>
    /// <exception cref="ArgumentException">Thrown when a MessageBus with the given name already exists.</exception>
    public static T Add<T>(string name) where T : MessageBusBase, new()
    {
        ...
        ReduxLib.Logger.LogDebug($"MessageBus '{name}' created");
        return messageBus;
    }
    ...
    internal static void CheckForMemoryLeaks()
    {
        ...
        if (memoryLeaks > 0)
        {
            ReduxLib.Logger.LogDebug($"{memoryLeaks} detected!");
        }
    }
}